...
# Set local python source root
path_root= Path(os.environ['HOME'] + "/text-classification/src/main/python")
sys.path.append(str(path_root))

from constants import defaults

# Facebook BART model fine-tuned for NLI tasks
nli_model = AutoModelForSequenceClassification.from_pretrained(defaults.HF_MNLI, truncation=True)
...
def simple_zero_shot_hf():
    ...
    print("HuggingFace classifier: ", hf_zero_shot_classify(sequence, defaults.YAHOO_CLASSES))
...
def simple_raw_nli():
    ...
    for label in defaults.YAHOO_CLASSES:
        ...
        print(f'HuggingFace zero-shot classification: {hf_zero_shot_classify(sequence, defaults.YAHOO_CLASSES)}')
        print()
